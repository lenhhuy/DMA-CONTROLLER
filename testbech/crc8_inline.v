...
module crc8_inline (
    ...
    // Đa thức G(x) = x^8 + x^2 + x + 1 (0x07), MSB-first, không reflection
    // Phương trình dẫn xuất: c = crc XOR data, rồi chạy 8 vòng LFSR
    // Đã kiểm tra: {0x31,0x32,0x33} với init=0xFF → 0xEB
    function [7:0] crc8_next;
        ...
        begin
            c = crc ^ d;
            crc8_next[7] = c[7]^c[6]^c[5];
            crc8_next[6] = c[6]^c[5]^c[4];
            crc8_next[5] = c[5]^c[4]^c[3];
            crc8_next[4] = c[4]^c[3]^c[2];
            crc8_next[3] = c[3]^c[2]^c[1]^c[7];
            crc8_next[2] = c[2]^c[1]^c[0]^c[6];
            crc8_next[1] = c[1]^c[0]^c[6];
            crc8_next[0] = c[0]^c[7]^c[6];
        end
    endfunction
